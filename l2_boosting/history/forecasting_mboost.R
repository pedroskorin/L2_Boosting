...
prediciton_boost = function(Y, X, v, h, ratio_start = 0.8, Mstop = 100) {
  ...
  prod_interno = function(x) {
    
    w_1 = match(x, m_1_coef)
    
    w_1_n = names(m_1_coef)[w_1]
    
    w_2 = match(w_1_n, colnames(x_reg))
    
    w_out = unlist(x) * unlist(x0_reg[,w_2])
  ...
  for(i in 1:n_out){
    ...
    x0_reg_df = as.data.frame(x0_reg)
    colnames(x0_reg_df) = paste("x_model", colnames(x_reg), sep = "")
    
    x_model = as.matrix(x_reg)
    colnames(x_model) = colnames(x_reg)
    
    model_1 = glmboost(y_reg ~ x_model,
                       family = Gaussian(),
                       control = boost_control(mstop = 5000, nu = v))
    
    m_1_coef = model_1$coef()
    
    names(m_1_coef) = gsub("x_model", "", names(model_1$coef()))
    
    coef_x_x0 = sapply(m_1_coef, prod_interno)
    
    cat(length(coef_x_x0), " TEM QUE SER IGUAL A ", length(m_1_coef), "\n")
    
    y_predicted = sum(unlist(coef_x_x0))
    
    aic_model_1 = AIC(model_1)
    
    M_otimo = readr::parse_number(capture.output(aic_model_1)[2])
    
    cat("O M ÓTIMO É", M_otimo, "\n")
    ...
    #management = append(management, model_1)
